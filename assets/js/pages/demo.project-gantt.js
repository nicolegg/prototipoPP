...
{ var e = new Gantt("#tasks-gantt", [
    ...
    { view_modes: ["Quarter Day", "Half Day", "Day", "Week", "Month"], bar_height: 20, padding: 18, view_mode: "Week", custom_popup_html: function (e) 
        { e.end; var s = 60 <= e.progress ? "bg-success" : 30 <= e.progress && e.progress < 60 ? "bg-primary" : "bg-warning"; return '<div class="popover fade show bs-popover-right gantt-task-details" role="tooltip"><div class="arrow"></div><div class="popover-body"><h5>' + e.name + '</h5><p class="mb-2">Expected to finish by ${end_date}</p><div class="progress mb-2" style="height: 10px;"><div class="progress-bar ' + s + '" role="progressbar" style="width: ${task.progress}%;" aria-valuenow="${task.progress}" aria-valuemin="0" aria-valuemax="100">${task.progress}%</div></div></div></div>' } }); $("#modes-filter :input").on("change", function () { e.change_view_mode($(this).val()) }); var s = document.getElementById("modes-filter").querySelectorAll(".btn"); s.forEach(function (e) { e.addEventListener("click", function () { s.forEach(function (e) 
